...
void lexor::setInputFile(const std::string &path)
{
	h.fileousHandler.connectFile(path);
	h.setFileName(path);
	h.fileousHandler.virgin = false;
	line = 1;
	column = 0;
	currentCharacter = h.fileousHandler.move();
	currentLexeme = "";
}
...
bool lexor::isReservedWord()
{
	...
	if (tokenMap.count(charAsString) > 0 || currentCharacter == '=')
		return true;
	else
		return false;
}
...
void lexor::setPossibleType()
{
	...
	std::string charAsString{currentCharacter};
	if (isInArray(letterArray))
		possibleType = "id";
	else if (isInArray(intArray))
		possibleType = "num";
...
std::unique_ptr<token> lexor::errorProtocol(std::string type)
{
	...
	if (type.compare("id") == 0)
	{
		...
		h.errorFileHandler.handleError("Lexical error", "Invalid Identifier", tempLexeme, line, column, t.get());
		return t;
	}
	else if (type.compare("intnum") == 0)
	{
		...
		h.errorFileHandler.handleError("Lexical error", "Invalid Number", tempLexeme, line, column, t.get());
		return t;
	}
	...
	else if (type.compare("invChar") == 0)
	{
		...
		h.errorFileHandler.handleError("Lexical error", "Invalid Character", tempLexeme, line, column, t.get());
		return t;
	}
...
std::unique_ptr<token> lexor::validToken(std::string type, int lineCounter)
{
	...
	if (type.compare("id") == 0)
	{
		if (isReservedWord(tempLexeme))
		{
			...
			h.fileousHandler.writeToken(t.get());
			return t;
		}
		else
		{
...
{
	while (currentCharacter != '\r' && currentCharacter != '\n' && !h.fileousHandler.checkEndOfStream())
	{
		addAndMove(tempLexeme);
	}
}
...
std::unique_ptr<token> lexor::id()
{
	...
	if (h.fileousHandler.checkEndOfStream() || isWhiteSpace() || isReservedWord())
	{
	}
	else
	{
...
std::unique_ptr<token> lexor::num(int decision)
{
	...
	if (currentCharacter == '0')
	{
		...
		if (decision == 0)
		{
			...
			if (h.fileousHandler.checkEndOfStream() || isWhiteSpace() || isReservedWord())
			{
				return validToken("intnum");
			}
			else
			{
				return errorProtocol("intnum");
			}
		}
		else if (decision == 1)
		{
...
std::unique_ptr<token> lexor::fraction()
{
	...
	if (isInArray(intArray))
	{
		...
		if (currentCharacter == 'e')
			return flt();
		else if (currentLexeme == "0.0")
		{
			return validToken("frac");
		}
		else if (currentLexeme.back() == '0' && currentLexeme.size() == 3)
		{
			return validToken("frac");
		}
		else if (currentLexeme.back() == '0')
		{
			return errorProtocol("frac");
		}

		// If we reach a white space or a reserve word we've reached the end of the lexeme
		else if (isWhiteSpace() || isReservedWord() || h.fileousHandler.checkEndOfStream())
		{
			return validToken("frac");
		}
		else
...
std::unique_ptr<token> lexor::res()
{
	...
	if (tokenMap.count(charAsString1) > 0 || currentCharacter == '=')
	{
		addAndMove();
	}
	else
...
std::unique_ptr<token> lexor::cmt()
{
	...
	else if (currentLexeme.compare("/*") == 0)
	{
		...
		while (true)
		{
			...
			while (currentCharacter != '*' && currentCharacter != '/' && !h.fileousHandler.checkEndOfStream())
			{
...
std::unique_ptr<token> lexor::getNextToken()
{
	...
	if (h.fileousHandler.virgin == true)
	{
		...
		if (!h.fileousHandler.virginProtocol())
	...
	if (possibleType == "id")
		return id();
	else if (possibleType == "num")
		return num(0);
	else if (possibleType == "res")
		return res();
	else if (possibleType == "cmt")
		return cmt();
	else if (possibleType == "inv")
